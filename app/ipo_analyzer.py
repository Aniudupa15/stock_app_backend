...
class IPOAnalyzer:
    """
    IPO tracking & analysis with cloud-safe storage.
    Uses /tmp directory (writable on Render & HuggingFace).
    Data resets on restart but never crashes.
    """

    def __init__(self, data_dir: str = "/tmp/ipo_data"):
        ...
        # Initialize safe JSON files
        self._init_files()
        self._initialize_sample_ipos()

    # ------------------------------------------------------------
    # JSON Load / Save (fully safe)
    # ------------------------------------------------------------

    def _init_files(self):
        """Initialize files if not present."""
        if not self.ipos_file.exists():
            self._safe_write(self.ipos_file, [])
        if not self.ipo_reminders_file.exists():
            self._safe_write(self.ipo_reminders_file, [])

    def _safe_read(self, filepath: Path):
        """Read JSON safely. Never crashes."""
        try:
    ...
    def _safe_write(self, filepath: Path, data: any):
        """Write JSON safely. Never crashes."""
        try:
            with open(filepath, "w") as f:
                json.dump(data, f, indent=2)
        except Exception as e:
    ...
    # ------------------------------------------------------------
    # Sample IPOs (always available)
    # ------------------------------------------------------------

    def _initialize_sample_ipos(self):
        """Create sample IPO if empty (useful after cold start)."""
        ipos = self._safe_read(self.ipos_file)
        if ipos:  # Already exists
            return
        ...
        self._safe_write(self.ipos_file, sample_ipos)
    ...
    def add_ipo(
        ...
        ipos.append(ipo)
        self._safe_write(self.ipos_file, ipos)

        return {"success": True, "message": "IPO added", "ipo": ipo}
    ...
    def get_ipos(self, status: Optional[str] = None, category: Optional[str] = None) -> List[Dict]:
        """Returns IPOs with optional filters."""
        ipos = self._safe_read(self.ipos_file)
    ...
    def get_upcoming_ipos(self, days: int = 30) -> List[Dict]:
        """IPOs opening within next N days."""
        ipos = self._safe_read(self.ipos_file)
        today = datetime.now().date()
        cutoff = today + timedelta(days=days)
        ...
        for ipo in ipos:
            try:
                open_date = datetime.strptime(ipo["open_date"], "%Y-%m-%d").date()
                if today <= open_date <= cutoff:
                    ipo["days_until_open"] = (open_date - today).days
                    upcoming.append(ipo)
            except:
                continue

        return sorted(upcoming, key=lambda x: x["open_date"])
    ...
    def analyze_ipo(self, ipo_id: int) -> Dict:
        ...
        if ipo["category"] == "Mainboard":
            analysis["score"] += 1
            analysis["factors"].append("Mainboard listing – better liquidity")
        else:
            analysis["risks"].append("SME IPO – lower liquidity")

        # Issue size logic
        if "Cr" in ipo["issue_size"]:
            try:
                size = float(ipo["issue_size"].replace("₹", "").replace("Cr", "").strip())
                if size >= 500:
                    analysis["score"] += 1
                    analysis["factors"].append("Large issue size – institutional interest likely")
            except:
                pass

        # Price band
        try:
            low, high = ipo["price_band"].replace("₹", "").split("-")
            if float(high) <= 500:
                analysis["opportunities"].append("Affordable for retail")
                analysis["score"] += 0.5
        except:
            pass
        ...
        return {"success": True, "ipo": ipo, "analysis": analysis}

    # ------------------------------------------------------------
    # Reminders
    # ------------------------------------------------------------

    def set_ipo_reminder(self, ipo_id: int, reminder_date: str, reminder_type: str = "OPENING", email: Optional[str] = None) -> Dict:
        reminders = self._safe_read(self.ipo_reminders_file)
        ipos = self._safe_read(self.ipos_file)
        ...
        reminders.append(reminder)
        self._safe_write(self.ipo_reminders_file, reminders)
    ...
    def get_ipo_calendar(self, month: Optional[int] = None) -> Dict:
        ...
        for ipo in ipos:
            try:
                # Opening
                open_dt = datetime.strptime(ipo["open_date"], "%Y-%m-%d")
                if open_dt.month == month and open_dt.year == year:
                    date_str = open_dt.strftime("%Y-%m-%d")
                    calendar.setdefault(date_str, []).append({
                        "company": ipo["company_name"],
                        "event": "OPENING",
                        "category": ipo["category"],
                        "price_band": ipo["price_band"]
                ...
                # Closing
                close_dt = datetime.strptime(ipo["close_date"], "%Y-%m-%d")
                if close_dt.month == month and close_dt.year == year:
