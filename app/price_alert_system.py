...
def _latest_close_price(symbol: str) -> Optional[float]:
    """
    Return latest close price using Alpha Vantage TIME_SERIES_DAILY_ADJUSTED.
    ...
    try:
        ...
        print(f"[price_alert] error fetching {symbol}: {e}")
        return None


class PriceAlertSystem:
    """
    Price alert monitoring system for stock price targets using Alpha Vantage.
    """
    def __init__(self, data_dir: str = "data"):
        self.data_dir = Path(data_dir)
        self.data_dir.mkdir(exist_ok=True)
    ...
    def create_alert(self, ticker: str, target_price: float, condition: str, email: Optional[str] = None, notes: Optional[str] = None) -> Dict:
        alerts = self._load_json(self.alerts_file)
        ticker = self.normalize_ticker(ticker)
        current_price = _latest_close_price(ticker) or 0.0
        alert = {
            "id": len(alerts) + 1,
            "ticker": ticker,
            "target_price": target_price,
        ...
        alerts.append(alert)
        self._save_json(self.alerts_file, alerts)
        return {"success": True, "message": f"Alert created for {ticker}", "alert": alert}
    ...
    def _group_by_ticker(self, alerts: List[Dict]) -> Dict:
        ...
        for alert in alerts:
            ticker = alert['ticker']
            grouped.setdefault(ticker, []).append(alert)
        return grouped
